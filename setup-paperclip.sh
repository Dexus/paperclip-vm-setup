...
# ---------- 6.5 register (or defer) allowed hostnames ---------------------
...
# `pnpm paperclipai allowed-hostname` only works AFTER `paperclip onboard`
# has created ~/.paperclip/instances/default/config.json (onboard is
# interactive — API keys etc. — so we can't run it from this script).
#
# Strategy: collect the hostnames we know about, then either register
# them now (if the config already exists from a previous run) or write a
# one-shot helper at ~/register-allowed-hostnames.sh that the operator
# runs once after `paperclip onboard`.
allowed_hosts=()
...
if (( ${#allowed_hosts[@]} > 0 )); then
  ...
  # Always drop the helper script so re-running setup-paperclip refreshes
  # the list as requirements change. The script is idempotent — registering
  # an already-registered hostname is a no-op on the Paperclip side.
  helper="${PAPERCLIP_HOME}/register-allowed-hostnames.sh"
  log "Writing ${helper}"
  {
    echo '#!/usr/bin/env bash'
    echo '# Generated by setup-paperclip.sh.'
    echo '# Run this AFTER `paperclip onboard` has created'
    echo '# ~/.paperclip/instances/default/config.json. Re-run any time you'
    echo '# need to refresh the allow-list.'
    echo 'set -euo pipefail'
    echo 'cd "$HOME/paperclip"'
    for h in "${allowed_hosts[@]}"; do
      printf 'pnpm paperclipai allowed-hostname %q\n' "$h"
    done
  } > "$helper"
  chown "$PAPERCLIP_USER:$PAPERCLIP_USER" "$helper"
  chmod 0755 "$helper"

  cfg="${PAPERCLIP_HOME}/.paperclip/instances/default/config.json"
  if [[ -f "$cfg" ]]; then
    ...
    for h in "${allowed_hosts[@]}"; do
      as_paperclip "cd \"\$HOME/paperclip\" && pnpm paperclipai allowed-hostname $(printf '%q' "$h")" \
        || warn "Could not register allowed-hostname '$h' — try ${helper} after fixing the issue."
    done
  else
    warn "Paperclip not yet onboarded ($cfg missing) — deferred hostname registration."
    warn "Once you've run 'paperclip onboard' as ${PAPERCLIP_USER}, register them with:"
    warn "    sudo -u ${PAPERCLIP_USER} -H bash ${helper}"
    warn "Hostnames queued: ${allowed_hosts[*]}"
  fi
fi
...
Next steps:
  ...
  2. Configure Paperclip itself:
       ...
       # After onboarding, register the hostnames we know about
       # (domain + WireGuard IP + extras you passed via
       # PAPERCLIP_ALLOWED_HOSTNAMES / --allowed-hostname):
       bash ~/register-allowed-hostnames.sh
